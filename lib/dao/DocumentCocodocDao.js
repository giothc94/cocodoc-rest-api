...
class DocumentCocodoc {
    ...
    deleteDocumentCocodoc = ({ idDoc }) => {
        const SQL = `DELETE FROM documents WHERE documents.id_doc = ?;`;
    ...
    getDocumentCocodoc = ({ idDoc }) => {
        const SQL = `SELECT f.path_system_folder,d.id_doc,d.name_doc FROM documents d inner join folders f on d.id_folder = f.id_folder where d.id_doc = ?;`
        return new Promise((resolve, reject) => {
            this.mysql.connection()
                .then(connection => {
                    connection.query(SQL, [idDoc], (error, resp) => {
