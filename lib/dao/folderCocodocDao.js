...
class FolderCocodoc {
  ...
  checkFolderExistenceById = async id => {
    return new Promise((resolve, reject) => {
      const SQL = `select exists(SELECT true FROM folders where folders.id_folder = ?) as "exist";`;
      this.mysql
        .connection()
        .then(connection => {
  ...
  checkFolderExistenceByPath = async path_user => {
    return new Promise((resolve, reject) => {
      ...
      this.mysql
        .connection()
        .then(connection => {
          connection.query(SQL, [normalize(path_user)], (error, resp) => {
            if (error) reject({ error: error });
  ...
  rootExist = () => {
    return new Promise((resolve, reject) => {
      const SQL = `SELECT count(*) as total FROM folders where is_root = 1;`;
      this.mysql
        .connection()
        .then(connection => {
          connection.query(SQL, (error, [resp]) => {
            ...
            if (total === 1) {
                let SQL = `SELECT * FROM folders where is_root = 1;`;
    ...
    return new Promise(async (resolve, reject) => {
      this.getFolderById(destinationFolderCode)
        .then(({ folder: { path_system_folder, path_user_folder } }) => {
          ...
          this._id = `${Date.now()}`;
          this._cod = this._id;
          this._pathSystem = normalize(path.join(
            `${path_system_folder}`,
            `${this._id}-${this._nameFolder}`
          ));
          this._pathUser = normalize(path.join(
            `${path_user_folder}`,
            `${this._nameFolder}`
          ));
  ...
  createRootFolder = async (nameFolder) => {
    return new Promise(async (resolve, reject) => {
        ...
        let rootFolder = normalize(path.join(DIR_NAME,nameFolder))
        ...
        .then(({ check }) => {
            if (check) {
                ...
                this._pathUser = normalize(`${this._nameFolder}`)
            return this.mysql.connection();
          }
        })
        .then(connection => {
