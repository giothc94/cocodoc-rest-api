            ...
            this.getFolderById(destinationFolderCode)
                .then(({ folder: { path_system_folder, path_user_folder } }) => {
                    ...
                    this.checkFolderExistenceByPath(this._pathUser)
                        .then(({ check }) => {
                            if (check) {
                                reject({ status: 400, message: "No se pudo crear la carpeta, ya existe una con el mismo nombre." });
                            } else {
            ...
            this.getFolderById(idFolder)
                .then(({ folder: { path_system_folder, path_user_folder } }) => {
                    ...
                    this.checkFolderExistenceByPath(this._pathUser)
                        .then(({ check }) => {
                            if (check) {
                                reject({ status: 400, message: "No se pudo renombrar la carpeta, ya existe una con el mismo nombre." });
                            } else {
