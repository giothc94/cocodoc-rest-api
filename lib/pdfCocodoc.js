...
class PDFCocodoc {
    ...
    createDoc({ nameFile, idFolderDestination, metadata, files, nameCreator }) {
        return new Promise(async(resolve, reject) => {
            try {
                const existFolder = await this.folderCocodoc.checkFolderExistenceById(idFolderDestination)
                if (existFolder) {
                    const { folder: { path_system_folder, path_user_folder } } = await this.folderCocodoc.getFolderById(idFolderDestination);
                    const doc = new PDFDoc({ autoFirstPage: false });
                    const id = Date.now();
                    const name = `${id}-${nameFile.replace(' ','_').trim().toLowerCase()}.pdf`;
